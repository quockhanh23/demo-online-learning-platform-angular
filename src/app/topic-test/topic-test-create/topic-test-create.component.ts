...
export class TopicTestCreateComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.topicTestService.getDetailTopicTestByLesson(this.idLesson).subscribe(rs => {
      this.topicTest = rs
  ...
  removeItem(index: number) {
    ...
    // 🔁 Cập nhật lại questionNumber sau khi xóa
    this.items.controls.forEach((ctrl, i) => {
      ctrl.get('questionNumber')?.setValue(i + 1);
    });
  }

  removeItem2(index: number) {
    if (index < 0 || index >= this.items2.length) return;

    this.items2.removeAt(index);

    // 🔁 Cập nhật lại questionNumber sau khi xóa
    this.items2.controls.forEach((ctrl, i) => {
      ctrl.get('questionNumber')?.setValue(i + 1);
    });
  }
